<template>
<div>
    <Alert type="success" show-icon banner>温馨提示：该图数据来源自合同模块！</Alert>
    <div v-show="showFlag" id="container"></div>
</div>
</template>
            ...
            getAntvVoList().then(res => {
                if (res.success) {
                    if (res.result.length < 1) {
                        that.showFlag = false;
                        this.$Message.warning("您的账号下没有合同哦！");
                    } else {
                        that.showFlag = true;
                    }
                ...
                statistic: {
                    title: {
                        ...
                        customHtml: (container, view, datum) => {
                            ...
                            const text = datum ? datum.title : '合同状态分布';
